...
const List = ({ cats }) => {
    ...
    console.log(router);
    const { id } = router.query; //query: {id: 'aege'}
    // console.log(cats);
    return (
        <div className={photosListStyle.photos}>
            <HeaderInfo tiltle="Cats World - search" />
            <h1>
                Cats Photo List : {id}{" "}
                <Link href="/photos">
                    <a>Go Back</a>
...
// getStaticPaths를 작성해야 합니다.
// 예를 들어 : photos/id 를 전송시
// getStaticPaths 가 실행 된다.
// params 를 통해서 전달된 id 를
// {params : {id :문자열값}} 을
// 모은 배열 (paths) 에 담아서 context 로 전송한다.
// 이렇게 전송된 context는 getStaticProps 에서 활용한다.
